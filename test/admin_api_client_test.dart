...
class MockAdminApiClient {
  ...
  Map<String, dynamic> getUsers({int page = 1, int limit = 50, String? role, String? status, String? plan, String? q}) {
  ...
  Map<String, dynamic> patchUserStatus(String targetUserId, String newStatus, {String? reason}) {
    final authCheck = _authorize();
    if (authCheck['statusCode'] != 200) return authCheck;
    ...
    if (adminUserId == targetUserId && newStatus != 'active') {
      return {
        ...
        'body': {
          'success': false,
          'error': 'Self-lockout prevented: You cannot suspend or deactivate your own admin account.',
          'code': 'FORBIDDEN',
        }
      };
    }
  ...
  Map<String, dynamic> patchUserPlan(String targetUserId, String newPlan, {int days = 30, String? reason}) {
    final authCheck = _authorize();
    if (authCheck['statusCode'] != 200) return authCheck;
  ...
  Map<String, dynamic> getAuditLogs({int page = 1, int limit = 50, String? action, String? resourceType}) {
    final authCheck = _authorize();
    if (authCheck['statusCode'] != 200) return authCheck;
          ...
          'logs': [
            {
              ...
              'changes': {'old_status': 'active', 'new_status': 'suspended', 'reason': 'Violation'},
            ...
            },
            {
              ...
              'resource_type': 'user',
              'resource_id': 'target-user-123',
              'changes': {'old_plan': 'free', 'new_plan': 'premium', 'days': 30},
...
void main() {
  group('ZankoAI Secure Admin System & API Tests', () {
    ...
    test('1. Role Authorization Enforcement: Only role=admin can access Admin APIs', () {
      final studentClient = MockAdminApiClient(userRole: 'student', adminUserId: 'student-id');
      final adminClient = MockAdminApiClient(userRole: 'admin', adminUserId: adminId);
    ...
    test('2. GET /api/admin/users: Lists users with pagination and filters; never returns raw credentials', () {
      final adminClient = MockAdminApiClient(userRole: 'admin', adminUserId: adminId);
      final res = adminClient.getUsers(page: 1, limit: 10, role: 'student', status: 'active');

      expect(res['statusCode'], equals(200));
      final data = res['body']['data'] as Map<String, dynamic>;
      final users = (data['users'] as List).map((u) => AdminUserRecord.fromJson(u)).toList();
    ...
    test('3. PATCH /api/admin/users/:id/status: Suspend & Activate with self-lockout protection', () {
      final adminClient = MockAdminApiClient(userRole: 'admin', adminUserId: adminId);

      // A: Suspend a target user -> Success
      final suspendRes = adminClient.patchUserStatus(targetUserId, 'suspended', reason: 'Abuse detected');
      ...
      final activateRes = adminClient.patchUserStatus(targetUserId, 'active', reason: 'Reinstated after review');
      ...
      final selfLockoutRes = adminClient.patchUserStatus(adminId, 'suspended');
      expect(selfLockoutRes['statusCode'], equals(403));
      expect(selfLockoutRes['body']['error'].toString().contains('Self-lockout prevented'), isTrue);
    });

    test('4. PATCH /api/admin/users/:id/plan: Controlled backend operation updates plan and VIP', () {
      final adminClient = MockAdminApiClient(userRole: 'admin', adminUserId: adminId);

      final planRes = adminClient.patchUserPlan(targetUserId, 'premium', days: 30, reason: 'Scholarship grant');
      expect(planRes['statusCode'], equals(200));
      expect(planRes['body']['data']['newPlan'], equals('premium'));
      expect(planRes['body']['data']['isVip'], isTrue);
    });

    test('5. GET /api/admin/usage: Returns MAU, DAU, AI requests, and estimated financial telemetry', () {
      final adminClient = MockAdminApiClient(userRole: 'admin', adminUserId: adminId);
    ...
    test('6. GET /api/admin/audit-logs: Validates immutable audit trails for sensitive operations', () {
      final adminClient = MockAdminApiClient(userRole: 'admin', adminUserId: adminId);

      final auditRes = adminClient.getAuditLogs(action: 'status_changed');
      expect(auditRes['statusCode'], equals(200));

      final logs = (auditRes['body']['data']['logs'] as List).map((l) => AdminAuditRecord.fromJson(l)).toList();
