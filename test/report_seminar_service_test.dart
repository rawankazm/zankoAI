...
void main() {
  ...
  group('Report & Seminar Supabase Models Test', () {
    test('AcademicSectionModel serializes and deserializes correctly', () {
      final section = AcademicSectionModel(
        ...
        content: 'Artificial Intelligence is revolutionizing clinical diagnostic workflows.',
      );
    ...
    test('AcademicReportRecord serializes and deserializes correctly with sections', () {
      ...
      expect(parsed.title, equals('ڕۆڵی ژیریی دەستکرد لە پەرەپێدانی سۆفتوێر'));
      expect(parsed.sections.length, equals(2));
      expect(parsed.sections[0].title, equals('پێشەکی و گرنگی بابەت'));
    });

    test('AcademicSeminarRecord serializes and deserializes correctly with slides/sections', () {
      final now = DateTime.now();
      ...
      expect(json['title'], equals('سیمیناری مایکرۆسێرڤس و دابەشکردنی سیستەمەکان'));
  ...
  group('ReportSeminarService Offline & Fallback Caching Test', () {
    test('createReport creates fallback and caches locally on network failure', () async {
    ...
    test('createSeminar creates fallback and caches locally on network failure', () async {
      final service = ReportSeminarService.instance;
  ...
  group('Prompt 38 Architectural Guardrails & Security Test', () {
    ...
    test('Supabase migration exists and includes reports, seminars, and RLS', () {
      ...
      expect(sql.contains('CREATE TABLE IF NOT EXISTS public.report_sections'), isTrue);
      expect(sql.contains('CREATE TABLE IF NOT EXISTS public.seminars'), isTrue);
      expect(sql.contains('CREATE TABLE IF NOT EXISTS public.seminar_sections'), isTrue);
      expect(sql.contains('CREATE TABLE IF NOT EXISTS public.academic_files'), isTrue);
