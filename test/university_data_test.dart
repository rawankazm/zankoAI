...
void main() {
  group('Kurdistan Universities & Departments Data Tests', () {
    test(
      'Universities list should contain all major institutions including colleges & institutes',
      () {
    ...
    test(
      'findUniversity should properly locate university or institute by name',
      () {
        ...
        final epu = KurdistanUniversitiesData.findUniversity(
    ...
    test(
      'getDepartmentsFor should return real academic majors for institutes',
      () {
        final shaqlawaDepts = KurdistanUniversitiesData.getDepartmentsFor(
          'کۆلێژی تەکنیکی شەقڵاوە (EPU)',
        );
        expect(
          shaqlawaDepts.any((d) => d.contains('شیکاری نەخۆشییەکان')),
          isTrue,
        );
        expect(shaqlawaDepts.any((d) => d.contains('پەرستاری')), isTrue);
        expect(
          shaqlawaDepts.any((d) => d.contains('تەکنەلۆجیای زانیاری')),
          isTrue,
        );
        expect(shaqlawaDepts.any((d) => d.contains('کارگێڕی کار')), isTrue);

        final fallback = KurdistanUniversitiesData.getDepartmentsFor(
          'Unknown College',
        );
    ...
    test(
      'getLocalizedUniversityName should translate properly across en, ar, ku',
      () {
        // English
        expect(
        ...
        // Kurdish
        final kuUni = KurdistanUniversitiesData.getLocalizedUniversityName(
          'Salahaddin University - Erbil',
          'ku',
        );
        expect(
    ...
    test(
      'getLocalizedDepartmentName should translate properly across en, ar, ku',
      () {
        // English
        expect(
          KurdistanUniversitiesData.getLocalizedDepartmentName(
            'کۆلێژی زانست - بەشی تەکنەلۆجیای زانیاری',
            'en',
          ),
    ...
    test('All polytechnic colleges have specialized technical majors', () {
      final epuEng = KurdistanUniversitiesData.findUniversity(
      ...
      final epuComp = KurdistanUniversitiesData.findUniversity(
        'کۆلێژی تەکنیکی ئەندازیاری کۆمپیوتەر',
      );
      expect(epuComp, isNotNull);
      expect(
        epuComp!.departments.any(
          (d) => d.contains('ڕۆبۆتیکس') || d.contains('ژیری دەستکرد'),
        ),
        isTrue,
      );
