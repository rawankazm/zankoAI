...
Map<String, dynamic> _buildHomeworkPayload({
  ...
  return {
    ...
    'data': {
      ...
      'relatedConcepts':
          relatedConcepts ??
          ['Free fall', 'Uniform acceleration', 'Kinematic equations'],
      'language': language,
      'hasImage': hasImage,
...
void main() {
  ...
  test(
    '1. Text-only homework submission — happy path receives 200 with structured output',
    () async {
  ...
  test(
    '2. Multimodal homework submission — text + image receives 200 with structured output',
    () async {
      final payload = _buildHomeworkPayload(
      ...
      final res = await mockDio.post<Map<String, dynamic>>(
        ...
        data: FormData.fromMap({
          'text':
              'Identify the major reaction product from the reaction in the image.',
          'subject': 'Organic Chemistry',
          'course': 'CHEM 201',
          'difficulty': 'hard',
        }),
  ...
  test(
    '3. Educational fields verification — contains all 6 required components',
    () async {
      final payload = _buildHomeworkPayload();
  ...
  test(
    '4. Omission of hidden chain-of-thought — response is concise and contains no raw scratchpad',
    () async {
      final payload = _buildHomeworkPayload();
  ...
  test(
    '6. Input size limit — text exceeding 5000 characters rejected with 400',
    () async {
      when(
        () => mockDio.post<Map<String, dynamic>>(
          '/ai/homework',
      ...
      expect(
        () => mockDio.post<Map<String, dynamic>>(
          '/ai/homework',
          data: {'text': oversizedText, 'subject': 'Computer Science'},
        ),
        throwsA(
          predicate(
            (e) =>
  ...
  test(
    ...
    () async {
        ...
        DioException(
          ...
          response: Response(
            ...
            data: {
              'success': false,
              'message':
                  'پێویستە دەق یان وێنەی پرسیار بنێریت بۆ ئەوەی شیکار بکرێت.',
              'code': 'BAD_REQUEST',
            },
          ),
        ),
      ...
      expect(
        () => mockDio.post<Map<String, dynamic>>(
          '/ai/homework',
          data: {'subject': 'Physics', 'difficulty': 'medium'},
        ),
        throwsA(
          predicate(
            (e) =>
  ...
  test(
    '8. Fake image magic bytes — disguised binary file rejected with 400',
    () async {
      when(
        () => mockDio.post<Map<String, dynamic>>(
          '/ai/homework',
  ...
  test(
    '9. Dimension & decompression bomb check — oversized pixels rejected with 400',
    () async {
      when(
        () => mockDio.post<Map<String, dynamic>>(
          '/ai/homework',
  ...
  test(
    '10. Abuse / prompt injection — malicious payload blocked with 400',
    () async {
      when(
        () => mockDio.post<Map<String, dynamic>>(
          '/ai/homework',
        ...
        DioException(
          ...
          response: Response(
            ...
            data: {
              'success': false,
              'message':
                  'داواکارییەکە ڕەتکرایەوە بەهۆی بوونی دەستەواژەی قەدەغەکراو (Prompt Injection detected).',
  ...
  test(
    '11. Rate limit enforcement — returns 429 when max requests per minute exceeded',
    () async {
      when(
        () => mockDio.post<Map<String, dynamic>>(
          '/ai/homework',
        ...
        DioException(
          ...
          response: Response(
            ...
            data: {
              'success': false,
              'message':
                  'زۆر داواکاری لە کاتێکی کەمدا ئەنجامدراوە. تکایە چاوەڕوان بە.',
  ...
  test(
    '12. Daily quota limit — returns 429 with QUOTA_EXCEEDED when daily limit reached',
    () async {
      when(
        () => mockDio.post<Map<String, dynamic>>(
          '/ai/homework',
        ...
        DioException(
          ...
          response: Response(
            ...
            data: {
              'success': false,
              'message':
                  'بەشی ڕۆژانەی شیکارکردنی پرسیار (Homework) تەواو بووە (10/10). تکایە هەژمارەکەت نوێبکەرەوە.',
              'code': 'QUOTA_EXCEEDED',
            },
          ),
        ),
  ...
  test(
    '13. Idempotency handling — duplicate request with same key returns cached solution',
    () async {
      ...
      final res1 = await mockDio.post<Map<String, dynamic>>(
        '/ai/homework',
        data: {
          'text': 'What is Merge Sort complexity?',
          'subject': 'Algorithms',
        },
  ...
  test(
    '15. Data model helpers — difficultyLabelKu and stepsCount format accurately',
    () {
      final easyModel = HomeworkSolutionModel.fromJson(
        _buildHomeworkPayload(difficulty: 'easy')['data'],
      );
      expect(easyModel.difficultyLabelKu, equals('ئاسان'));

      final medModel = HomeworkSolutionModel.fromJson(
        _buildHomeworkPayload(difficulty: 'medium')['data'],
      );
      expect(medModel.difficultyLabelKu, equals('مامناوەند'));

      final hardModel = HomeworkSolutionModel.fromJson(
        _buildHomeworkPayload(difficulty: 'hard')['data'],
      );
      expect(hardModel.difficultyLabelKu, equals('سەخت'));

      final advModel = HomeworkSolutionModel.fromJson(
        _buildHomeworkPayload(difficulty: 'advanced')['data'],
      );
