...
void main() {
  ...
  group('1. Active Subscription Lifecycle State', () {
    test('User with active period has isPremium == true and can access features', () async {
      final futureEnd = DateTime.now().add(const Duration(days: 28));

      when(() => mockDio.get<Map<String, dynamic>>('/subscription'))
  ...
  group('2. Expired Lifecycle State & Strict Rule', () {
    test('STRICT RULE: If current_period_end < now, user cannot be treated as Premium', () async {
          ...
          .thenAnswer((_) async => _buildResponse(
                ...
                data: {
                  ...
                  'data': {
                    ...
                    'plan': 'FREE',
                    'status': 'expired',
                    'currentPeriodStart': DateTime.now().subtract(const Duration(days: 32)).toIso8601String(),
      ...
      expect(sub.isPremium, isFalse, reason: 'Strict rule: period ended means not premium');
      ...
      when(() => mockDio.post<Map<String, dynamic>>(
            ...
            data: {
              ...
              'data': {
                ...
                'message': 'Subscription will remain active until period end and will not renew.',
              },
            },
  ...
  group('4. Failed Renewal & Grace Period (past_due)', () {
    test('Failed renewal transitions to past_due with active grace period', () async {
      ...
      expect(sub.canAccessPremiumFeatures, isFalse,
          reason: 'STRICT RULE: If period has ended, user cannot be treated as Premium');
    });
  });

  group('5. Renewed Lifecycle State', () {
    test('Renewal seamlessly extends period without losing remaining days', () async {
  ...
  group('6. Auto-Charge Protection Rule', () {
    test('Non-recurring providers (Qi Card/ZainCash/FastPay) and unauthorized users block auto-charge', () {
      const nonRecurringProvider = 'qi_card';
      const autoRenewAuthorized = false;

      // Business logic condition:
      final canAutoCharge = nonRecurringProvider == 'stripe' && autoRenewAuthorized;
      expect(canAutoCharge, isFalse, reason: 'Iraqi gateways do not support recurring auto-charge');
    });
  });

  group('7. GET /api/subscription/history Endpoint', () {
    test('Fetches aggregated subscription, payment, and event history', () async {
      when(() => mockDio.get<Map<String, dynamic>>('/subscription/history'))
          .thenAnswer((_) async => _buildResponse(
                statusCode: 200,
                data: {
                  'success': true,
                    ...
                    'subscriptions': [
                      {
                        ...
                        'plan': 'PREMIUM_MONTHLY',
                        'status': 'active',
                        'current_period_end': DateTime.now().add(const Duration(days: 25)).toIso8601String(),
                      }
                    ],
                    'payments': [
                      {
