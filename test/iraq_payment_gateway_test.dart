...
void main() {
  ...
  group('1. Successful Payment & Checkout', () {
    test(
      'initiates checkout with Qi Card and returns valid hosted payment URL',
      () async {
        when(
          ...
          return _buildResponse(
            ...
            data: {
              ...
              'data': {
                ...
                'paymentUrl':
                    'https://api.qicard.net/v1/checkout/order_1725700000_qicard',
                'status': 'pending',
                'amount': 15000.0,
                'currency': 'IQD',
              },
  ...
  group('2. Failed Payment Handling', () {
    test(
      'payment record reflects failed status when gateway reports payment failure',
      () async {
        when(
          () => mockDio.get<Map<String, dynamic>>('/payments/order_fail_01'),
        ).thenAnswer(
  ...
  group('3. Duplicate Webhook Idempotency', () {
    test(
      'idempotency logic identifies replayed webhooks and prevents duplicate crediting',
      () {
        ...
        expect(
          firstAttempt,
          isFalse,
          reason: 'First arrival should be processed',
        );
        ...
        expect(
          secondAttempt,
          isTrue,
          reason: 'Duplicate arrival must be detected',
        );
      },
    );
  });
  ...
  group('4. Invalid Webhook Signature Rejection', () {
    test(
      'rejects forged webhook callback when HMAC or token signature does not match',
      () {
  ...
  group('5. Wrong Amount Rejection', () {
    test(
      'rejects webhook when paid amount does not match expected plan price',
      () {
  ...
  group('7. Expired Transaction Handling', () {
    test(
      'marks payment as cancelled when session expires at the gateway',
      () async {
        when(
          () => mockDio.get<Map<String, dynamic>>('/payments/order_expired_01'),
        ).thenAnswer(
          (_) async => _buildResponse(
            statusCode: 200,
  ...
  group('8. Cancelled Payment Handling', () {
    test(
      'properly serializes cancelled status when customer cancels checkout',
      () {
        final json = {
  ...
  group('9. Premium Activation Upon Verified Payment', () {
    test(
      'verified paid record marks isPaid to true and ready for VIP grant',
      () async {
        when(
          () => mockDio.get<Map<String, dynamic>>('/payments/order_success_01'),
        ).thenAnswer(
          (_) async => _buildResponse(
            statusCode: 200,
  ...
  group('10. Iraqi Provider Manual Renewal Flow', () {
    test(
      'manual renewal checkout passes isRenewal: true to extend existing subscription',
      () async {
        when(
          () => mockDio.post<Map<String, dynamic>>(
            '/payments/checkout',
          ...
          return _buildResponse(
            ...
            data: {
              ...
              'data': {
                ...
                'paymentUrl':
                    'https://secure.fast-pay.iq/checkout?order_id=order_renewal_777',
                'status': 'pending',
                'amount': 15000.0,
                'currency': 'IQD',
              },
