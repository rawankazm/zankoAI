...
void main() {
  ...
  group('1. AI Request Size Caps & 413 Payload Too Large Handling', () {
    test('Client receives clear error when request size exceeds tier limits', () async {
      ...
      try {
        await mockDio.post<Map<String, dynamic>>('/ai/chat', data: {'prompt': 'x' * 5000});
  ...
  group('2. AI Concurrency Limits (429 AI_CONCURRENCY_LIMIT_EXCEEDED)', () {
    test('Client gracefully handles parallel concurrency lock rejection', () async {
      when(() => mockDio.post<Map<String, dynamic>>(
            '/ai/homework',
            data: any(named: 'data'),
            options: any(named: 'options'),
      ...
      try {
        await mockDio.post<Map<String, dynamic>>('/ai/homework', data: {'subject': 'Math'});
        fail('Should have thrown DioException');
      } on DioException catch (e) {
        expect(e.response?.statusCode, equals(429));
        final errorData = e.response?.data?['error'] as Map<String, dynamic>?;
        expect(errorData?['code'], equals('AI_CONCURRENCY_LIMIT_EXCEEDED'));
        expect(errorData?['message'], contains('wait for your previous request to finish'));
      }
    });
  });
  ...
  group('3. AI Spending & Budget Enforcement (Daily / Monthly Cutoffs)', () {
    test('Client detects and presents daily expenditure budget limit', () async {
      ...
      try {
        await mockDio.post<Map<String, dynamic>>('/ai/chat', data: {'prompt': 'Hello'});
        fail('Should have thrown DioException');
      } on DioException catch (e) {
        expect(e.response?.statusCode, equals(429));
        final errorData = e.response?.data?['error'] as Map<String, dynamic>?;
        expect(errorData?['code'], equals('AI_DAILY_BUDGET_EXCEEDED'));
        expect(errorData?['message'], contains('Daily AI expenditure budget exceeded'));
      }
    });

    test('Client detects and presents monthly expenditure budget limit', () async {
      when(() => mockDio.post<Map<String, dynamic>>(
            '/ai/generate-quiz',
            data: any(named: 'data'),
            options: any(named: 'options'),
      ...
      try {
        await mockDio.post<Map<String, dynamic>>('/ai/generate-quiz', data: {'topic': 'Biology'});
        fail('Should have thrown DioException');
      } on DioException catch (e) {
        expect(e.response?.statusCode, equals(429));
        final errorData = e.response?.data?['error'] as Map<String, dynamic>?;
        expect(errorData?['code'], equals('AI_MONTHLY_BUDGET_EXCEEDED'));
        expect(errorData?['message'], contains('Monthly AI expenditure budget exceeded'));
      }
    });
  });

  group('4. Secret Protection & Sanitization', () {
    test('Error messages from backend contain only redacted tokens, never raw API keys', () async {
      ...
      try {
        await mockDio.post<Map<String, dynamic>>('/ai/chat', data: {'prompt': 'Hi'});
        fail('Should have thrown DioException');
      } on DioException catch (e) {
        final errorData = e.response?.data?['error'] as Map<String, dynamic>?;
