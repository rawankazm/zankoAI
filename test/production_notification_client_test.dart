...
void main() {
  ...
  group('1. Production Notification Types Parsing', () {
    test('parses all 8 required production types accurately from string', () {
      expect(AppNotificationType.fromString('assignment_reminder'),
          equals(AppNotificationType.assignmentReminder));
      expect(AppNotificationType.fromString('exam_reminder'),
          equals(AppNotificationType.examReminder));
      expect(AppNotificationType.fromString('announcement'),
          equals(AppNotificationType.announcement));
      expect(AppNotificationType.fromString('teacher_announcement'),
          equals(AppNotificationType.teacherAnnouncement));
      expect(AppNotificationType.fromString('ai_job_completion'),
          equals(AppNotificationType.aiJobCompletion));
      expect(AppNotificationType.fromString('subscription_activated'),
          equals(AppNotificationType.subscriptionActivated));
      expect(AppNotificationType.fromString('subscription_expiring'),
          equals(AppNotificationType.subscriptionExpiring));
      expect(AppNotificationType.fromString('payment_result'),
          equals(AppNotificationType.paymentResult));
      expect(AppNotificationType.fromString('system_notification'),
      ...
      expect(AppNotificationType.announcement.toSnakeCase(), equals('announcement'));
      expect(AppNotificationType.aiJobCompletion.toSnakeCase(), equals('ai_job_completion'));
      expect(AppNotificationType.subscriptionActivated.toSnakeCase(), equals('subscription_activated'));
      expect(AppNotificationType.subscriptionExpiring.toSnakeCase(), equals('subscription_expiring'));
      expect(AppNotificationType.paymentResult.toSnakeCase(), equals('payment_result'));
      expect(AppNotificationType.systemNotification.toSnakeCase(), equals('system_notification'));

      // Kurdish display names
      expect(AppNotificationType.aiJobCompletion.displayNameKu, contains('ژیری دەستکرد'));
      expect(AppNotificationType.subscriptionActivated.displayNameKu, contains('VIP'));
      expect(AppNotificationType.paymentResult.displayNameKu, contains('پارەدان'));
    });
  });

  // ─── 2. Notification Model JSON Mapping ─────────────────────────────────────
  group('2. AppNotificationModel Data Mapping', () {
    test('deserializes complete production notification object with read_at', () {
  ...
  group('6. Device Registration & Removal Client Endpoints', () {
    ...
    test('DELETE /devices/:id unregisters device upon logout', () async {
      when(() => mockDio.delete<Map<String, dynamic>>('/devices/device-row-uuid-999'))
          .thenAnswer((_) async => Response(
                statusCode: 200,
                data: {'success': true, 'message': 'Device unregistered successfully'},
                requestOptions: RequestOptions(path: '/devices/device-row-uuid-999'),
              ));

      await expectLater(
        service.deleteDevice('device-row-uuid-999'),
        completes,
      );
    });

    test('GET /devices returns list of active user devices', () async {
      when(() => mockDio.get<Map<String, dynamic>>('/devices'))
          .thenAnswer((_) async => Response(
                statusCode: 200,
                data: {
                  'success': true,
