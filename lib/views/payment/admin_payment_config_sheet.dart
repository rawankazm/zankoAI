...
class _AdminPaymentConfigSheetState extends State<AdminPaymentConfigSheet> {
  ...
  Future<void> _savePaymentNumbers() async {
    ...
    if (fib.isEmpty || fastpay.isEmpty || zaincash.isEmpty) {
      setState(() => _statusMsg = 'تکایە سەرجەم ژمارەکانی پارەدان بنووسە');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('ژمارەکانی پارەدان بە سەرکەوتوویی پاشەکەوت کران 💳'),
      ...
      if (mounted) {
        setState(() {
          _isSaving = false;
          _statusMsg = 'کێشەیەک هاتە لە پاشەکەوتکردن: $e';
        });
      }
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  const Text(
                    '💳 ڕێکخستنی ژمارەکانی پارەدان (Admin Panel)',
              ...
              Text(
                'ئەم ژمارانەی خوارەوە بگۆڕە تا ڕاستەوخۆ بۆ هەموو خوێندکاران لە بەشی پارەدانی VIP ڕەنگ بداتەوە:',
